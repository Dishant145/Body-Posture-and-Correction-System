...
def gen(value):
    ...
    # eng.setProperty('voice', voice[0].id) #set the voice to index 0 for male voice
            ...
            if bad_time > 4 and value == 1:
                win32api.MessageBox(0, 'Please Straighten Your Head!', 'Alert', 0x00001000) 
                ...
                voice = engine.getProperty('voices') 
                engine.setProperty('voice', voice[1].id)
                text = "Please Straighten Your Head!"  
                engine.say(text)  
                engine.runAndWait()
            ...
            cv2.putText(image, str(int(neck_inclination)), (l_shldr_x + 10, l_shldr_y), font, 0.9, red, 2)
            cv2.putText(image, str(int(torso_inclination)), (l_hip_x + 10, l_hip_y), font, 0.9, red, 2)
            # cv2.putText(image, "Please Straighten Your Head and Sit Straight", (240, 320), font, 0.9, light_green, 2)
            # # Join landmarks.
            cv2.line(image, (l_shldr_x, l_shldr_y), (l_ear_x, l_ear_y), red, 4)
            cv2.line(image, (l_shldr_x, l_shldr_y), (l_shldr_x, l_shldr_y - 100), red, 4)
            cv2.line(image, (l_hip_x, l_hip_y), (l_shldr_x, l_shldr_y), red, 4)
